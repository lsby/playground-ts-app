...
let 接口逻辑实现 = 接口逻辑
  ...
  .绑定(
    接口逻辑.构造(
      [
        new JSON参数解析插件(z.object({ 任务id: z.string() }).strict(), {}),
        new WebSocket插件(
          z.object({ 新日志: z.object({ 时间: z.number(), 消息: z.string() }).strict() }).strict(),
          z.object({}).strict(),
        ),
      ],
...
let 接口正确类型描述 = z
  .object({ 日志列表: z.array(z.object({ 时间: z.number(), 消息: z.string() }).strict()) })
  .strip()
