...
function 获得纯前端Worker(): Worker {
  if (纯前端Worker === undefined) {
    纯前端Worker = new Worker(new URL('../../pure-frontend/pure-frontend-api-worker.ts', import.meta.url), {
      type: 'module',
      name: `${项目标识}-pure-frontend-sqlite`,
    })
...
export async function 请求纯前端Worker(
  ...
  return 接口响应模式.parse(JSON.parse((await 请求纯前端Worker响应(消息)).body))
}
