...
- 机制与控制：
  - 由环境变量 `PREVIEW_MODE` 控制, 有配套的启动入口与任务套件
  - 复用正式接口路由与出入参契约, 无需建立平行的 Mock 服务, 接口内部通过判断 `环境变量.PREVIEW_MODE` 走预览分支
- 编写原则与规范：
...
- 环境变量机制:
  ...
  - 工程任务直接指定环境文件路径, 初始化流程按 `.example` 文件名创建对应本地文件. `NODE_ENV` 与 `BUILD_TARGET` 在加载时校验运行环境, `LOCAL_MODE` 控制本地免登录, `PREVIEW_MODE` 控制选中业务接口的预览分支
